...
export function useActivityTracking() {
  ...
  const submitToLayerTwoLabs = useCallback(async (activity: ActivitySession): Promise<boolean> => {
    if (!layerTwoLabsService.isServiceInitialized()) {
      console.log("LayerTwoLabs service not initialized, skipping submission")
      return false
    }
    ...
    try {
      ...
      console.error("Error submitting to LayerTwoLabs:", error)
      return false
    }
  }, [])
  ...
  const stopActivity = useCallback(() => {
    ...
    // Submit to LayerTwoLabs if enabled
    submitToLayerTwoLabs(completedActivity)
      .then((success) => {
        if (success) {
          console.log("Activity successfully submitted to LayerTwoLabs")
        }
      })
      .catch((error) => {
        console.error("Error in LayerTwoLabs submission:", error)
      })

    return completedActivity
  }, [isTracking, currentActivity, watchId, stats, locations, submitToLayerTwoLabs])
